...
assert_identical() {
  cmp -s artifacts/realcv-lanes-status.v1.json artifacts/realcv-crosslane-status.json
}
...
run_case() {
  ...
  [[ -f artifacts/realcv-lanes-status.v1.json ]]
  [[ -f artifacts/realcv-crosslane-status.json ]]
  assert_identical
}
...
python3 - <<'PY'
import json
s=json.load(open('/tmp/pt-orion/artifacts/realcv-lanes-status.v1.json'))
